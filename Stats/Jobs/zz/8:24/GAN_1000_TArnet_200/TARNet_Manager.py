...
class TARNet_Manager:
    ...
    def train_semi_supervised(self, train_parameters,
                              val_parameters,
                              n_total, n_treated, device):
        ...
        data_loader_train = torch.utils.data.DataLoader(tensor_dataset,
        ...
        early_stopping = EarlyStopping_DCN(patience=100, verbose=True,
        ...
        for epoch in range(epochs):
            ...
            for batch in data_loader_train:
                covariates_X, ps_score, T, y_f = batch
                ...
                y_f_treated = y_f[idx]
                y_f_treated = y_f_treated.long()

                covariates_X_control = covariates_X[~idx]
                y_f_control = y_f[~idx]
                y_f_control = y_f_control.long()
                ...
                if treated_size > 0:
                    ...
                    if torch.cuda.is_available():
                        loss_T = weight_t * lossF(y1_hat.cuda(),
                                                  y_f_treated.cuda()).to(device)
                    else:
                        loss_T = weight_t * lossF(y1_hat,
                                                  y_f_treated).to(device)
                    loss_T.backward()
                ...
                if control_size > 0:
                    ...
                    if torch.cuda.is_available():
                        loss_C = weight_c * lossF(y0_hat.cuda(),
                                                  y_f_control.cuda()).to(device)
                    else:
                        loss_C = weight_c * lossF(y0_hat,
                                                  y_f_control).to(device)
                    loss_C.backward()
    ...
    def eval(self, eval_parameters, device):
        ...
        for batch in treated_data_loader:
            ...
            covariates_X = covariates_X.to(device)
            pred_y1_hat = self.tarnet_h_y1(self.tarnet_phi(covariates_X))
            pred_y0_hat = self.tarnet_h_y0(self.tarnet_phi(covariates_X))
